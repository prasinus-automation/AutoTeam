...
- **Architect re-triage state key (`architect-retriage-{N}`)** is cleared by `_handle_agent_success` (success path) and by `_handle_agent_failure` (both transient and non-transient paths) for any architect-role completion. If you add new event paths that can trigger re-triage, audit `state.clear_handled(...)` carefully — without an explicit clear, a second unblock event for the same issue within one daemon lifetime is silently dropped by the `state.already_handled('architect-retriage-N')` guard.
